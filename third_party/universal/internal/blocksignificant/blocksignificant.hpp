#pragma once
// blocksignificant.hpp: parameterized blocked binary number system representing
// the bits of the floating-point significant scaled for the different
...
/*
   ...
   - for division
   - for square root
   a blocksignificant type will be marked by its encoding to enable direct code
   paths. By encoding it in the type, we won't be able to dynamically go between
   types, but that is ok as the blocksignificant is a composition type that gets
   used by the ephemeral blocktriple type, which is set up for each
   floating-point operation, used, and then discarded.

   The last piece of information we need to manage for blocksignificants is
   where the radix point is. For add/sub it is at a fixed location, nbits - 3,
   and for multiplication and division is transforms from the input values to
   the output values. The blocksignificant operators, add, sub, mul, div, sqrt
   manage this radix point transformation. Fundamentally, the actual bits of the
   blocksignificant are used as a binary encoded integer. The encoding
   interpretation and the placement of the radix point, are directed by the
   aggregating class, such as blocktriple.
 */
namespace sw {
namespace universal {
...
// structure for blocksignificant<nbits> to capture quotient and remainder
...
This disqualifies using uint64_t as a block type as we can't catch the overflow
condition in the same way as the other native types, uint8_t, uint16_t,
uint32_t. We could use a sint64_t and then convert to uint64_t and observe the
MSB. That requires different logic though.
TODO: the highest performance for 32bits and up would be to have a uint64_t base
type for which we need asm to get the carry bit logic to work.

TODO: are there mechanisms where we can use SIMD for vector operations?
If there are, then doing something with more fitting and smaller base types
might yield more concurrency and thus more throughput for that ISA.


NOTE 2
adding two block triples of nbits would yield a result of nbits+1. To implement
a fast use of blocksignificant storage complicates this relationship.

Standardizing the blocktriple add to take two arguments of nbits, and product a
result of nbits+1, makes sense in the abstract pipeline as the triple would gain
one bit of accuracy. Any subsequent use would need to make a decision whether to
round or not. If we go to a quire, we wouldn't round, if we reassign it to a
source precision, we would.
...
///
/// NOTE: don't set a default blocktype as this makes the integration more
/// brittle as blocktriple uses the blocksignificant as storage class and needs
/// to interact with the client number system, which also is blocked. Using the
/// same blocktype simplifies the copying of exponent and fraction bits from and
/// to the client.
/// </summary>
/// <typeparam name="bt"></typeparam>
template <unsigned _nbits, typename bt> class blocksignificant {
public:
  static constexpr unsigned nbits = _nbits;
  static constexpr unsigned bitsInByte = 8;
  static constexpr unsigned bitsInBlock = sizeof(bt) * bitsInByte;
  static_assert(bitsInBlock <= 64,
                "storage unit for block arithmetic needs to be <= uint64_t");

  static constexpr unsigned nrBlocks = 1ull + ((nbits - 1ull) / bitsInBlock);
  static constexpr uint64_t storageMask =
      (0xFFFF'FFFF'FFFF'FFFFull >> (64 - bitsInBlock));
  static constexpr uint64_t maxRightShift =
      ((64 - nbits + 3) > 62) ? 63 : (64 - nbits + 3);
  static constexpr uint64_t fmask = (64 - nbits + 3) > 63
                                        ? 0ull
  ...
  // constructors
  constexpr blocksignificant() noexcept
      : radixPoint{nbits}, encoding{BitEncoding::Flex}, _block{} {}

  // value constructors
  constexpr blocksignificant(signed char rhs) noexcept
      : radixPoint{nbits}, encoding{BitEncoding::Ones}, _block{} {}
  constexpr blocksignificant(int rhs) noexcept
      : radixPoint{nbits}, encoding{BitEncoding::Ones}, _block{} {}

  // raw bit constructors
  template <size_t... I>
  constexpr blocksignificant(const uint64_t raw, int radixPoint,
  ...
  constexpr blocksignificant(const uint64_t raw, int radixPoint) noexcept
      : blocksignificant(raw, radixPoint,
                         std::make_index_sequence<nrBlocks>{}) {}
  ...
  operator=(const blocksignificant &) noexcept = default;
  ...
  // uses in add/sub/mul/div/sqrt will directly access the bits of the
  // encapsulated blocksignificant.

  /// construct a blocksignificant from another: bt must be the same
  template <unsigned nnbits>
  blocksignificant(const blocksignificant<nnbits, bt> &rhs) {
    this->assign(rhs);
  }
  ...
  constexpr blocksignificant(long long initial_value) noexcept : _block{0} {
    *this = initial_value;
  }
  ...
  explicit constexpr operator double() const noexcept {
    return double(to_double());
  }

#if LONG_DOUBLE_SUPPORT
  explicit constexpr operator long double() const noexcept {
    return (long double)to_long_double();
  }
  ...
  /// logic operators
  constexpr bool
  anyAfter(unsigned bitIndex) const noexcept { // TODO: optimize for limbs
    if (bitIndex < nbits) {
      for (unsigned i = 0; i < bitIndex; ++i)
        if (test(i))
          return true;
    }
    return false;
  ...
  constexpr void add(uint8_t v) noexcept {
    bool carry = true;
    for (unsigned i = 0; i < nrBlocks; ++i) {
  ...
  void div(const blocksignificant &lhs, const blocksignificant &rhs) noexcept {
    ...
    for (unsigned i = 0; i <= 2 * fbits; ++i) {
      //			std::cout << "base    : " << to_binary(base) <<
      //" : " << base << '\n'; 			std::cout << "divider : " << to_binary(divider)
      //<< " : " << divider << '\n';
      if (divider <= base) {
  ...
  constexpr blocksignificant &operator<<=(int bitsToShift) noexcept {
    if (bitsToShift == 0)
      return *this;
    if (bitsToShift < 0)
      return operator>>=(-bitsToShift);
    if (bitsToShift > long(nbits))
      bitsToShift = nbits; // clip to max
  ...
  constexpr blocksignificant &operator>>=(int bitsToShift) noexcept {
    ...
    if constexpr (MSU > 0) {
      bt mask = ALL_ONES;
      mask >>= (bitsInBlock -
                bitsToShift); // this is a mask for the lower bits in the block
                              // that need to move to the lower word
      for (unsigned i = 0; i < MSU;
           ++i) { // TODO: can this be improved? we should not have to work on
                  // the upper blocks in case we block shifted
        _block[i] >>= bitsToShift;
  ...
  constexpr void setbits(uint64_t value) noexcept {
    // radixPoint needs to be set, either using the constructor or the
    // setradix() function
    if constexpr (1 == nrBlocks) {
  ...
  constexpr uint8_t nibble(unsigned n) const noexcept {
    if (n < (1 + ((nbits - 1) >> 2))) {
      ...
      return static_cast<uint8_t>(nibblebits >>
                                  static_cast<bt>(nibbleIndexInWord * 4));
    }
  ...
  template <size_t... I>
  constexpr uint64_t
  significant_ull(std::index_sequence<I...> = {}) const noexcept {
    uint64_t raw{};
    raw = _block[MSU];
    raw &= MSU_MASK;
    if constexpr (sizeof...(I) == 0) {
      if constexpr (bitsInBlock < 64 && nrBlocks > 1) {
        return blocksignificant::significant_ull(
            std::make_index_sequence<MSU>{});
      } else { // if bitsInBlock < 64, take top 64bits and ignore the rest
        return raw;
      }
    } else {
      return ((raw <<= bitsInBlock, raw |= _block[MSU - 1 - I]), ...);
    }
  }
  ...
  inline blocksignificant<nbits, bt> &
  assign(const blocksignificant<srcbits, bt> &rhs) noexcept {
    clear();
    // since bt is the same, we can directly copy the blocks in
    unsigned minNrBlocks =
        (this->nrBlocks < rhs.nrBlocks) ? this->nrBlocks : rhs.nrBlocks;
    for (unsigned i = 0; i < minNrBlocks; ++i) {
      _block[i] = rhs.block(i);
    }
    if constexpr (nbits > srcbits) { // check if we need to sign extend
      if (rhs.sign()) {
        for (unsigned i = srcbits; i < nbits;
             ++i) { // TODO: replace bit-oriented sequence with block
          setbit(i);
        }
      }
    }
  ...
  // copy a value over from one blocksignificant to this without sign-extending
  // the value blocksignificant is a 2's complement encoding, so we sign-extend
  // by default for fraction/significent encodings, we need to turn off
  // sign-extending.
  template <unsigned srcbits>
  inline blocksignificant<nbits, bt> &
  assignWithoutSignExtend(const blocksignificant<srcbits, bt> &rhs) noexcept {
    clear();
  ...
  constexpr float to_float() const noexcept { return float(to_double()); }
  constexpr double to_double() const noexcept {
    ...
    // nbits in the target form 00h.fffff, check msb and if set take 2's
    // complement
    ...
    //		if constexpr (nbits > 49) { // check if we can represent this value
    //with a native normal double with 52 fraction bits => nbits <= (52 - 3)
    //			std::cerr << "to_double() will yield inaccurate result since
    //blocksignificant has more precision than native IEEE-754 double\n";
    //		}

    return s * d;
  }

  // determine the rounding direction for round-to-even: returns true if we need
  // to round up, false if we need to truncate Function argument is the bit
  // position of the LSB of the target number.
...
public:
  ...
  friend constexpr bool operator==(const blocksignificant &lhs,
                                   const blocksignificant &rhs) noexcept {
  ...
  friend constexpr blocksignificant operator<<(const blocksignificant &a,
                                               const long b) noexcept {
...
std::string to_binary(const blocksignificant<nbits, bt> &number,
...
template <unsigned nbits, typename bt>
std::string to_hex(const blocksignificant<nbits, bt> &number,
                   bool wordMarker = true) {
  static constexpr unsigned bitsInByte = 8;
  static constexpr unsigned bitsInBlock = sizeof(bt) * bitsInByte;
  char hexChar[16] = {
      '0', '1', '2', '3', '4', '5', '6', '7',
  ...
  for (int n = nrNibbles - 1; n >= 0; --n) {
    ...
    ss << hexChar[nibble];
    if (wordMarker && n > 0 && ((n * 4ll) % bitsInBlock) == 0)
      ss << '\'';
  }
  return ss.str();
}
...
template <unsigned nbits, typename bt>
bsquorem<nbits, bt> longdivision(const blocksignificant<nbits, bt> &_a,
                                 const blocksignificant<nbits, bt> &_b) {
  ...
  /*
          ...
          // 2's complement special case -max requires an signed int that is 1
     bit bigger to represent abs() bool a_sign = _a.sign(); bool b_sign =
     _b.sign(); bool result_negative = (a_sign ^ b_sign);
          // normalize both arguments to positive, which requires expansion by
     1-bit to deal with maxneg blocksignificant<nbits + 1, bt> a(_a);
...
inline blocksignificant<2 * nbits + roundingBits, bt>
urdiv(const blocksignificant<nbits, bt> &a,
      const blocksignificant<nbits, bt> &b,
      blocksignificant<roundingBits, bt> &r) {
  if (b.iszero()) {
    // division by zero
    throw "urdiv divide by zero";
  }
  // generate the absolute values to do long division
  // 2's complement special case -max requires an signed int that is 1 bit
  // bigger to represent abs()
  bool a_sign = a.sign();
  bool b_sign = b.sign();
  bool result_negative = (a_sign ^ b_sign);

  // normalize both arguments to positive in new size
  blocksignificant<nbits + 1, bt> a_new(
      a); // TODO optimize: now create a, create _a.bb, copy, destroy _a.bb_copy
  blocksignificant<nbits + 1, bt> b_new(b);
  if (a_sign)
    a_new.twoscomplement();
  if (b_sign)
    b_new.twoscomplement();

  // initialize the long division
  blocksignificant<2 * nbits + roundingBits, bt> decimator(a_new);
  blocksignificant<2 * nbits + roundingBits, bt> subtractand(
      b_new); // prepare the subtractand
  blocksignificant<2 * nbits + roundingBits, bt> result;

  int msp = nbits + roundingBits - 1; // msp = most significant position
  decimator <<=
      msp; // scale the decimator to the largest possible positive value
  ...
  std::cout << "- " << to_binary(subtractand) << " shift: " << shift
            << std::endl;
#endif
  // long division
...
#if TRACE_DIV
    std::cout << "  " << to_binary(decimator) << ' ' << to_binary(result)
              << std::endl;
  ...
  r.assign(result); // copy the lowest bits which represent the bits on which we
                    // need to apply the rounding test
  return result;
}

// free function generator of the 2's complement of a blocksignificant
template <unsigned nbits, typename bt>
inline constexpr blocksignificant<nbits, bt>
twosComplementFree(const blocksignificant<nbits, bt> &a) noexcept {
