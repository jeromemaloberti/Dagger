#pragma once
// blocktriple.hpp: definition of a (sign, scale, significant) representation of
// a generic floating-point value that goes into an arithmetic operation
//
...
// should be defined by calling environment, just catching it here just in case
// it is not
...
blocksignificant = 00h.ffffeee <- three bits before radix point, fraction bits
plus 3 rounding bits unsigned bfbits = fbits + 3;
...
class blocktriple;
template <unsigned fbits, BlockTripleOperator op, typename bt>
blocktriple<fbits, op, bt> abs(const blocktriple<fbits, op, bt> &v);

template <unsigned fbits, BlockTripleOperator op, typename bt>
blocktriple<fbits, op, bt> &convert(unsigned long long uint,
                                    blocktriple<fbits, op, bt> &tgt) {
  return tgt;
}
...
std::string type_tag(const blocktriple<fbits, op, bt> & = {}) {
  std::stringstream s;
  s << "blocktriple<" << std::setw(3) << fbits << ", " << op << ", "
    << typeid(bt).name() << '>';
  return s.str();
}

/// <summary>
/// Generalized blocktriple representing a (sign, scale, significant) with
/// unrounded arithmetic
///
/// For addition and subtraction, blocktriple uses a 2's complement
/// representation of the form iii.fffff. The 3 integer bits are required to
/// capture the negative overflow condition.
///
/// For multiplication, blocktriple uses a 1's complement representation of the
/// form ii.fffff. The 2 integer bits are required to capture the overflow
/// condition.
///
/// Blocktriple does not normalize the output of ADD/SUB/MUL so that all bits
/// are available for the rounding decision. Number systems that use blocktriple
/// as their general floating-point engine can use the roundUp(targetFbits)
/// method to obtain the rounding direction, and the alignmentShift(targetFbits)
/// method to obtain the shift required to normalize the fraction bits.
/// </summary>
/// <typeparam name="fbits">number of fraction bits in the
/// significant</typeparam> <typeparam name="bt">block type: one of [uint8_t,
/// uint16_t, uint32_t, uint64_t]</typeparam>
template <unsigned _fbits, BlockTripleOperator _op = BlockTripleOperator::ADD,
          typename bt = std::uint32_t>
class blocktriple {
public:
  static constexpr unsigned fbits =
      _fbits; // a convenience and consistency alias
  ...
  static constexpr uint64_t storageMask =
      (0xFFFF'FFFF'FFFF'FFFFull >> (64ull - bitsInBlock));

  static constexpr unsigned MSU =
      nrBlocks - 1ull; // MSU == Most Significant Unit, as MSB is already taken

  static constexpr unsigned fhbits = fbits + 1; // size of all bits
  static constexpr unsigned rbits =
      3; // rounding bits assumes you have sticky bit consolidation in
         // normalize, otherwise you need 2 * (fbits + 1) to capture the tie
         // breaking ULPs
  static constexpr unsigned abits =
      fbits + rbits; // size of the addend = fbits plus an additional rbits to
                     // capture required rounding bits
  static constexpr unsigned mbits =
      2 * fbits; // size of the fraction bits of the multiplier
  static constexpr unsigned divbits =
      3 * fbits + 4; // size of the fraction bits of the divider
  static constexpr unsigned divshift =
      divbits - fbits; // alignment shift for divider operands
  static constexpr unsigned sqrtbits =
  ...
  static constexpr unsigned bfbits =
      (op == BlockTripleOperator::ADD
           ? (3 + abits)
           : // we need 3 integer bits (bits left of the radix point) to capture
             // 2's complement and overflow
           (op == BlockTripleOperator::MUL
                ? (2 + mbits)
                : // we need 2 integer bits to capture overflow: multiply
                  // happens in 1's complement
                (op == BlockTripleOperator::DIV
                     ? (2 + divbits)
                     : // we need 2 integer bits to capture overflow: divide
                       // happens in 1's complement
                     (op == BlockTripleOperator::SQRT
                          ? sqrtbits
                          : fhbits +
                                1)))); // REPRESENTATION is the fall through
                                       // condition and adds a bit to accomodate
                                       // 2's complement encodings
  // radix point of the OUTPUT of an operator
  static constexpr int radix =
      (op == BlockTripleOperator::ADD
           ? static_cast<int>(abits)
           : (op == BlockTripleOperator::MUL
                  ? static_cast<int>(mbits)
                  : (op == BlockTripleOperator::DIV
                         ? static_cast<int>(divbits)
                         : (op == BlockTripleOperator::SQRT
                                ? static_cast<int>(sqrtbits)
                                : static_cast<int>(
                                      fbits))))); // REPRESENTATION is the fall
                                                  // through condition
  ...
  static constexpr uint64_t normalFormMask =
      (normalBits == 64) ? 0xFFFF'FFFF'FFFF'FFFFull
                         : (~(0xFFFF'FFFF'FFFF'FFFFull << (normalBits - 1)));
  // to maximize performance, can we make the default blocktype a uint64_t?
  // storage unit for block arithmetic needs to be uin32_t until we can figure
  // out how to manage carry propagation on uint64_t using intrinsics/assembly
  // code
  using Significant = sw::universal::blocksignificant<bfbits, bt>;

  static constexpr bt ALL_ONES = bt(~0);
  // generate the special case overflow pattern mask when representation is
  // fbits + 1 < 64
  static constexpr unsigned maxbits = (fbits + 1) < 63 ? (fbits + 1) : 63;
  static constexpr uint64_t overflowPattern =
      (maxbits < 63) ? (1ull << maxbits)
                     : 0ull; // overflow of 1.11111 to 10.0000
  ...
  constexpr blocktriple() noexcept
      : _nan{false}, _inf{false}, _zero{true}, _sign{false}, _scale{0} {
  } // _significant uses default constructor and static constexpr radix
    // computation
  ...
  constexpr blocktriple &operator=(unsigned int rhs) noexcept {
    return convert_unsigned_integer(rhs);
  }
  ...
  constexpr blocktriple &operator=(float rhs) noexcept {
    return convert_ieee754(rhs);
  }
  constexpr blocktriple &operator=(double rhs) noexcept {
    return convert_ieee754(rhs);
  }
  ...
  CONSTEXPRESSION blocktriple &assign(const std::string &bitPattern) noexcept {
    ...
    if (nrChars > 2 && bitPattern[0] == '0' && bitPattern[1] == 'b') {
      ...
      std::cerr << "bitPattern must start with 0b: instead input pattern was "
                << bitPattern << '\n';
      return *this;
    }
    ...
    if (nrBits != bfbits) {
      std::cerr << "nr of bits in bitPattern is " << nrBits
                << " and needs to be " << bfbits << '\n';
      return *this;
    }
    // assign the bits
  ...
  CONSTEXPRESSION blocktriple &operator=(long double rhs) noexcept {
    return convert_ieee754(rhs);
  }
  ...
  /// <summary>
  /// roundingDecision returns a pair<bool, unsigned> to direct the rounding and
  /// right shift
  /// </summary>
  /// <param name="adjustment">adjustment for subnormals </param>
  /// <returns>std::pair<bool, unsigned> of rounding direction (up is true, down
  /// is false), and the right shift</returns>
  constexpr std::pair<bool, unsigned>
  roundingDecision(int adjustment = 0) const noexcept {
  ...
  void adjustAddition() {
    ...
    // find the shift that gets us to the lsb
    unsigned shift = significantScale + static_cast<unsigned>(radix) - fbits;
    bool roundup = _significant.roundingDirection(shift);
    if (roundup) {
      _significant.add(8);
    }
    if (significantScale > 0) {
      _scale += significantScale;
      _significant >>= significantScale;
    }
    // reset the rounding bits
    _significant.setbit(0, false);
    _significant.setbit(1, false);
    _significant.setbit(2, false);
  }
  ...
  constexpr void setradix(int _radix) noexcept {
    _significant.setradix(_radix);
  }
  constexpr void setbit(unsigned index, bool v = true) noexcept {
    _significant.setbit(index, v);
  }
  constexpr void setbits(uint64_t raw) noexcept {
    // the setbits() api cannot be modified as it is shared by all number
    // systems as a standard mechanism for the test suites to set bits. However,
    // blocktriple uses extra state to encode the special values, and the test
    // can't use this interface to set that. Thus the caller (typically the test
    // suite) must manage this special state. _scale must be set by caller, so
    // that the same raw bit pattern can span different scales
    //
  ...
  constexpr void setblock(unsigned i, const bt &block) noexcept {
  ...
  inline constexpr Significant significant() const noexcept {
    return _significant;
  }
  inline constexpr Significant fraction() const noexcept {
    return _significant.fraction();
  }
  inline constexpr uint64_t significant_ull() const noexcept {
    return _significant.significant_ull();
  } // fast path when bfbits <= 64 to get the significant bits out of the
    // representation
  inline constexpr uint64_t fraction_ull() const noexcept {
    return _significant.fraction_ull();
  }
  inline constexpr bool at(unsigned index) const noexcept {
    return _significant.at(index);
  }
  inline constexpr bool test(unsigned index) const noexcept {
    return _significant.at(index);
  }
  inline constexpr bool any(unsigned index) const noexcept {
    return _significant.any(index);
  }
  inline constexpr bt block(unsigned b) const noexcept {
    return _significant.block(b);
  }
  ...
  void constexprClassParameters() const {
    ...
    std::cout << "rbits             : " << rbits
              << "      rounding bits for addition/subtraction\n";
    std::cout << "abits             : " << abits
              << "      size of the addend = fbits + rbits\n";
    std::cout << "mbits             : " << mbits
              << "      size of the multiplier output\n";
    std::cout << "divbits           : " << divbits
              << "      size of the divider output\n";
    std::cout << "sqrtbits          : " << sqrtbits
              << "      size of the square root output\n";
    // we transform input operands into the operation's target output size
    // so that everything is aligned correctly before the operation starts.
    std::cout << "bfbits            : " << bfbits
              << "      bits in the blocksignificant representation\n";
    std::cout
        << "radix             : " << radix
        << "      position of the radix point of the ALU operator result\n";
    //		std::cout << "encoding          : " << encoding << '\n';
    std::cout << "normalBits        : " << normalBits
              << "      normal bits to track: metaprogramming trick to remove "
                 "warnings\n";
    std::cout << "normalFormMask    : " << to_binary(normalFormMask)
              << "   normalFormMask for small configurations\n";
    std::cout << "significant type  : " << typeid(Significant).name() << '\n';

    std::cout << "ALL_ONES          : " << to_binary(ALL_ONES) << '\n';
    std::cout << "maxbits           : " << maxbits
              << "        bit to check for overflow: metaprogramming trick\n";
  ...
  /////////////////////////////////////////////////////////////
  // ALU operators
  void normalize(int max_scale, int shift, bool with_sticky) {
    int scaleDiff = max_scale - scale() + shift;
    if (scaleDiff > 0) {
      bool sticky = with_sticky ? any(static_cast<unsigned>(scaleDiff)) : false;
      *this >>= scaleDiff;
      setbit(0, sticky);
    }
  }
  ...
  /// yielding an unrounded sum of 3+fbits. (currently we generate a 3+(2*fbits)
  /// result as we haven't implemented the sticky bit optimization) This sum can
  /// overflow, be normal, or denormal. Since we are not rounding we cannot act
  /// on overflow as we would potentially shift rounding state out, and thus the
  /// output must be processed by the calling environment. We can act on
  /// denormalized encodings, so these are processed in this function. To avoid
  /// fraction bit copies, the input arguments must be prepared by the calling
  /// environment, and this function only manipulates the bits.
  /// </summary>
  ...
  void add(blocktriple &lhs, blocktriple &rhs) {
    ...
    // sticky bit calculation: abits = 1 hidden, f fraction, and r rounding
    // bits: 1+f+r
    // lhs ->  h.ffffrrr     h = hidden, f = fraction, and r is
    // rounding bits
    // rhs ->  h.ffffrrr
    ...
    if (lhs.isneg())
      lhs._significant.twosComplement();
    if (rhs.isneg())
      rhs._significant.twosComplement();

    _significant.add(lhs._significant,
                     rhs._significant); // do the bit arithmetic manipulation
    _significant.setradix(radix); // set the radix interpretation of the output

    if constexpr (_trace_btriple_add) {
      std::cout
          << "blocksignificant unrounded add: just the significant values\n";
      std::cout << typeid(_significant).name() << '\n';
      std::cout << "lhs significant : " << to_binary(lhs._significant) << " : "
                << lhs._significant << '\n';
      std::cout << "rhs significant : " << to_binary(rhs._significant) << " : "
                << rhs._significant << '\n';
      std::cout << "sum significant : " << to_binary(_significant) << " : "
                << _significant << '\n';
    }
  ...
  void add(blocktriple &lhs, blocktriple &rhs, int max_scale, bool normalize) {
    int scale_of_result = max_scale;
    if (lhs.isneg())
      lhs._significant.twosComplement();
    if (rhs.isneg())
      rhs._significant.twosComplement();
    ...
    if (_significant.iszero()) {
      ...
      _zero = false;
      // std::cerr << "Debug " << (bfbits - 1) << " "
      //           << _significant.test(bfbits - 1) << " "
      //           << to_binary(_significant) << std::endl;
      if (_significant.test(bfbits - 1)) { // is the result negative?
        _significant.twosComplement();
        // std::cerr << "Debug2c " << to_binary(_significant) << std::endl;
        _sign = true;
      }
      _scale = scale_of_result;
      // leave 01#.ffff to output processing: this is an overflow condition
      // 001.ffff is a perfect normalized format
      // fix 000.#### denormalized state to normalized
      if (normalize && !_significant.test(bfbits - 2) &&
          !_significant.test(bfbits - 3)) {
        // found a denormalized form to normalize: find MSB
        int msb = _significant.msb(); // zero case has been taken care off above
        int leftShift = static_cast<int>(bfbits) - 3 - msb;
        _significant <<= leftShift;
  ...
  void mul(blocktriple &lhs, blocktriple &rhs) {
    ...
    // avoid copy by directly manipulating the fraction bits of the arguments
    _significant.mul(lhs._significant,
                     rhs._significant); // do the bit arithmetic manipulation
    _significant.setradix(2 *
                          fbits); // set the radix interpretation of the output
    ...
    if constexpr (_trace_btriple_mul) {
      ...
      std::cout << "rhs significant : " << to_binary(rhs._significant) << " : "
                << rhs._significant << '\n';
      std::cout << "mul significant : " << to_binary(_significant) << " : "
                << _significant << '\n';
    }
    if (_significant.iszero()) {
      clear();
      ...
      // that is left to the conversion stage were we need to apply rounding
      // rules
  ...
  void div(blocktriple &lhs, blocktriple &rhs) {
    ...
    if constexpr (_trace_btriple_div) {
      ...
      std::cout << "rhs significant : " << to_binary(rhs._significant) << " : "
                << rhs._significant << '\n';
      std::cout << "div significant : " << to_binary(_significant) << " : "
                << _significant
...
private:
  ...
  constexpr uint64_t round(uint64_t raw) noexcept {
    if constexpr (fbits < srcbits) {
      ...
      // float significant: 24bits : 0bhfff'ffff'ffff'ffff'ffff'ffff; h is
      // hidden, f are fraction bits blocktriple target: 10bits: 0bhfff'ffff'fff
      // hidden bit is implicit, 10 fraction bits
      //                                           lg'rs
      //                             0b0000'0000'0001'0000'0000'0000; guard mask
      //                             == 1 << srcbits - fbits - 2: 24 - 10 - 2 =
      //                             12
      constexpr unsigned upper = ieee754_parameter<Real>::nbits + 2;
      constexpr int offset = fbits + 2;
      constexpr int fullShift =
          srcbits - offset; // srcbits includes the hidden bit, fbits does not
      constexpr unsigned shift = (fullShift < 0 ? 0 : fullShift);
      uint64_t mask = (srcbits < offset ? 0 : (1ull << shift));
      //			std::cout << "raw     : " << to_binary(raw, 64,
      // true) << '\n'; 			std::cout << "guard   : " <<
      // to_binary(mask, 64, true) <<
      //'\n';
      bool guard = (mask & raw);
      mask >>= 1;
      //			std::cout << "round   : " << to_binary(mask, 64,
      // true) << '\n';
      bool round = (mask & raw);
      if constexpr (shift > 1 &&
                    shift < upper) { // protect against a negative shift
      ...
      //			std::cout << "sticky  : " << to_binary(mask, 64,
      // true) << '\n';
      ...
      //			std::cout << "upshift : " << to_binary(raw, 64,
      // true) << '\n';
    }
    return raw;
  }
  ...
  constexpr inline blocktriple &
  convert_unsigned_integer(const Ty &rhs) noexcept {
    clear();
    _nan = false;
    _inf = false;
    _zero = true;
    ...
    _scale = static_cast<int>(findMostSignificantBit(raw)) -
             1; // precondition that msb > 0 is satisfied by the zero test above
  ...
  constexpr inline blocktriple &convert_signed_integer(const Ty &rhs) noexcept {
    ...
    uint64_t shift = sizeInBits - _scale - 1;
    raw <<= shift;
    uint64_t rounded_bits = round<sizeInBits, uint64_t>(
        raw); // TODO: there is something wrong here: that second template param
  ...
  inline CONSTEXPRESSION blocktriple &
  convert_ieee754(Real rhs) noexcept { // TODO: deal with subnormals and inf
    clear();
    ...
    if (rawExponent == 0ull) {
      ...
      uint64_t rounded_bits =
          round<ieee754_parameter<Real>::fbits + 1, Real>(rawFraction);
      _significant.setbits(rounded_bits);
      switch (op) {
      case BlockTripleOperator::REP:
        _significant.setradix(fbits);
  ...
  template <typename Real> constexpr Real to_native() const noexcept {
    if (_nan) {
      if (_sign)
  ...
  friend std::istream &operator>>(std::istream &istr,
                                  blocktriple<ffbits, oop, bbt> &a);

  // declare as friends to avoid needing a marshalling function to get
  ...
  // logic operators
  template <unsigned ffbits, BlockTripleOperator oop, typename bbt>
  friend bool operator==(const blocktriple<ffbits, oop, bbt> &lhs,
                         const blocktriple<ffbits, oop, bbt> &rhs);
  template <unsigned ffbits, BlockTripleOperator oop, typename bbt>
  friend bool operator!=(const blocktriple<ffbits, oop, bbt> &lhs,
                         const blocktriple<ffbits, oop, bbt> &rhs);
  template <unsigned ffbits, BlockTripleOperator oop, typename bbt>
...
template <unsigned fbits, BlockTripleOperator op, typename bt>
inline std::ostream &operator<<(std::ostream &ostr,
                                const blocktriple<fbits, op, bt> &a) {
  if (a.isnan()) {
    if (a.isneg()) {
      ostr << "snan";
    } else {
...
template <unsigned fbits, BlockTripleOperator op, typename bt>
inline bool operator==(const blocktriple<fbits, op, bt> &lhs,
                       const blocktriple<fbits, op, bt> &rhs) {
  return lhs._sign == rhs._sign && lhs._scale == rhs._scale &&
         lhs._significant == rhs._significant && lhs._zero == rhs._zero &&
         lhs._inf == rhs._inf;
}
template <unsigned fbits, BlockTripleOperator op, typename bt>
inline bool operator!=(const blocktriple<fbits, op, bt> &lhs,
...
inline bool operator<(const blocktriple<fbits, op, bt> &lhs,
  ...
  if (lhs._inf) {
    if (rhs._inf)
      ...
      return true; // everything is less than -infinity
  } else {
    if (rhs._inf)
      return false;
  }
      ...
      if (lhs._scale > rhs._scale) {
        ...
        if (lhs._scale == rhs._scale) {
          ...
          if (lhs._significant == rhs._significant)
            return false; // they are the same value
          if (lhs._significant > rhs._significant) {
            return true; // lhs is more negative
          } else {
...
template <unsigned fbits, BlockTripleOperator op, typename bt>
std::string to_binary(const sw::universal::blocktriple<fbits, op, bt> &a,
                      bool nibbleMarker = false) {
  return to_triple(a, nibbleMarker);
}

template <unsigned fbits, BlockTripleOperator op, typename bt>
std::string to_triple(const blocktriple<fbits, op, bt> &a,
