#pragma once
// cfloat_impl.hpp: implementation of an arbitrary configuration fixed-size
// 'classic' floating-point representation cfloat<> can emulate IEEE-754 floats
// and the new Deep Learning types, such as IEEE-754 half-precision floats
// Google bfloat16
// NVIDIA TensorFloat
// AMD FP16 and FP32
 ...
 *
 * Behavior flags
 *   subnormals  : gradual underflow: use all fraction encodings when exponent
 * is all 0's supernormals: gradual overflow: use all fraction encodings when
 * exponent is all 1's saturation to maxneg or maxpos when value is out of
 * dynamic range
 */
...
void decode(const cfloat<nbits, es, bt, hasSubnormals, hasSupernormals,
                         isSaturating> &v,
            bool &s, blockbinary<es, bt> &e, blockbinary<fbitsPlus1, bt> &f) {
  v.sign(s);
  v.exponent(e);
  v.fraction(f);
}
...
/// </summary>
/// <typeparam name="bt">Block type used for storage: derived through
/// ADL</typeparam> <param name="v">the cfloat number for which we seek to know
/// the binary scale</param> <returns>binary scale, i.e. 2^scale, of the value
/// of the cfloat</returns>
template <unsigned nbits, unsigned es, typename bt, bool hasSubnormals,
          bool hasSupernormals, bool isSaturating>
int scale(const cfloat<nbits, es, bt, hasSubnormals, hasSupernormals,
                       isSaturating> &v) {
  return v.scale();
}
...
template <unsigned srcbits, BlockTripleOperator op, unsigned nbits, unsigned es,
          typename bt, bool hasSubnormals, bool hasSupernormals,
          bool isSaturating>
inline /*constexpr*/ void convert(
  ...
  if (src.isnan()) {
    ...
    if constexpr (hasSubnormals) {
      //			std::cout << "exponent = " << exponent << " bias
      //= " << cfloatType::EXP_BIAS << " exp subnormal = " <<
      // cfloatType::MIN_EXP_SUBNORMAL << '\n';
      // why must exponent be less than (minExpSubnormal - 1) to be rounded to
      // zero? because the half-way value that would round up to minpos is at
      // exp = (minExpSubnormal - 1)
      if (exponent < cfloatType::MIN_EXP_SUBNORMAL) {
        tgt.setzero();
        if (exponent == (cfloatType::MIN_EXP_SUBNORMAL - 1)) {
          // -exponent because we are right shifting and exponent in this range
          // is negative
          int adjustment = -(exponent + subnormal_reciprocal_shift[es]);
          std::pair<bool, unsigned> alignment =
              src.roundingDecision(adjustment);
          if (alignment.first)
            ++tgt; // we are minpos
        }
      ...
      if (exponent + cfloatType::EXP_BIAS <=
          0) { // value is in the subnormal range, which maps to 0
        tgt.setzero();
        tgt.setsign(src.sign());
        return;
      }
    ...
    } else { // no supernormals will saturate at a different encoding: TODO can
             // we hide it all in maxpos?
      if constexpr (isSaturating) {
        if (exponent > cfloatType::MAX_EXP) {
          if (src.sign())
            tgt.maxneg();
    ...
    //
    // tgt.clear();  // no need as all bits are going to be set by the code
    // below

    // exponent construction
    int adjustment{0};
    // construct exponent
    uint64_t biasedExponent = static_cast<uint64_t>(
        static_cast<long long>(exponent) +
        static_cast<long long>(
            cfloatType::EXP_BIAS)); // this is guaranteed to be positive if
                                    // exponent in encoding range
    //			std::cout << "exponent         " <<
    // to_binary(biasedExponent)
    //<<
    //'\n';
    if constexpr (hasSubnormals) {
      // if (exponent >= cfloatType::MIN_EXP_SUBNORMAL && exponent <
      // cfloat<nbits, es, bt, hasSubnormals, hasSupernormals,
      // isSaturating>::MIN_EXP_NORMAL) {
      if (exponent < cfloatType::MIN_EXP_NORMAL) {
        // the value is in the subnormal range of the cfloat
        biasedExponent = 0;
        // -exponent because we are right shifting and exponent in this range is
        // negative
        adjustment = -(exponent + subnormal_reciprocal_shift[es]);
        // this is the right shift adjustment required for subnormal
        // representation due to the scale of the input number, i.e. the
        // exponent of 2^-adjustment
      } else {
        // the value is in the normal range of the cfloat
        biasedExponent = static_cast<uint64_t>(
            static_cast<long long>(exponent) +
            static_cast<long long>(
                cfloatType::EXP_BIAS)); // this is guaranteed to be positive
      }
    } else {
      if (exponent < cfloatType::MIN_EXP_NORMAL)
        biasedExponent =
            1ull; // fixup biasedExponent if we are in the subnormal region
    }
    ...
    unsigned rightShift = alignment.second; // this is the shift to get the LSB
                                            // of the src to the LSB of the tgt
    ...
    if constexpr (btType::bfbits < 65) {
      ...
      uint64_t fracbits =
          src.significant_ull(); // get all the bits, including the integer bits
      ...
      tgt.setbits(raw);
      // std::cerr << "tgt " << to_binary(tgt) << std::endl;
      //			std::cout << "raw bits (all)   " <<
      // to_binary(raw) << '\n';
      // when you get too far, map it back to +-inf: TBD: this doesn't appear to
      // be the right algorithm to catch all overflow patterns
      if (tgt.isnan())
        tgt.setinf(src.sign()); // map back to +-inf
    } else {
      // compose the segments
      auto fracbits =
          src.significant(); // why significant? cheesy optimization: we are
                             // going to overwrite the hidden bit position
                             // anyway when we write the exponent below, so no
                             // need to pay the overhead of generating the
                             // fraction here.
      ...
      //   ADD        iii.ffffrrrrrrrrr          3 integer bits, f fraction
      //   bits, and 2*fhbits rounding bits MUL         ii.ffff'ffff 2 integer
      //   bits, 2*f fraction bits DIV         ii.ffff'ffff'ffff'rrrr    2
      //   integer bits, 3*f fraction bits, and r rounding bits
...
/// <summary>
/// An arbitrary, fixed-size floating-point number with configurable gradual
/// under/overflow and saturation/non-saturation arithmetic. Default
/// configuration offers normal encoding and non-saturating arithmetic.
/// /// </summary>
...
/// <typeparam name="bt">the type to use as storage class: one of
/// [uint8_t|uint16_t|uint32_t]</typeparam> <typeparam
/// name="hasSubnormals">configure gradual underflow (==subnormals)</typeparam>
/// <typeparam name="hasSupernormals">configure graudal overflow
/// (==supernormals)</typeparam> <typeparam name="isSaturating">configure
/// saturation arithmetic</typeparam>
template <unsigned _nbits, unsigned _es, typename bt = uint8_t,
          bool _hasSubnormals = false, bool _hasSupernormals = false,
          bool _isSaturating = false>
class cfloat {
public:
  static_assert(
      _nbits > _es + 1ull,
      "nbits is too small to accomodate the requested number of exponent bits");
  static_assert(
      _es < 21ull,
      "my God that is a big number, are you trying to break the Interweb?");
  static_assert(_es > 0, "number of exponent bits must be bigger than 0 to be "
                         "a classic floating point number");
  // how do you assert on the condition that if es == 1 then subnormals and
  // supernormals must be true?
  static constexpr bool subsuper = (_hasSubnormals && _hasSupernormals);
  static constexpr bool special = (subsuper ? true : (_es > 1));
  static_assert(
      special,
      "when es == 1, cfloat must have both subnormals and supernormals");
  static constexpr unsigned bitsInByte = 8u;
  static constexpr unsigned bitsInBlock = sizeof(bt) * bitsInByte;
  static_assert(bitsInBlock <= 64,
                "storage unit for block arithmetic needs to be <= "
                "uint64_t"); // TODO: carry propagation on uint64_t
                             // requires assembly code

  static constexpr unsigned nbits = _nbits;
  static constexpr unsigned es = _es;
  static constexpr unsigned fbits =
      nbits - 1u - es; // number of fraction bits excluding the hidden bit
  static constexpr unsigned fhbits =
      nbits - es; // number of fraction bits including the hidden bit

  static constexpr uint64_t storageMask =
      (0xFFFFFFFFFFFFFFFFull >> (64ull - bitsInBlock));
  ...
  static constexpr uint64_t ALL_ONES_FR =
      (topfbits > 0 ? (0xFFFF'FFFF'FFFF'FFFFull >> FR_SHIFT)
                    : 0ull); // special case for nbits <= 64
  static constexpr uint64_t INF_ENCODING = (ALL_ONES_FR & ~1ull);

  static constexpr unsigned nrBlocks = 1u + ((nbits - 1ull) / bitsInBlock);
  static constexpr unsigned MSU =
      nrBlocks - 1u; // MSU == Most Significant Unit, as MSB is already taken
  static constexpr bt MSU_MASK = (ALL_ONES >> (nrBlocks * bitsInBlock - nbits));
  static constexpr unsigned bitsInMSU =
      bitsInBlock - (nrBlocks * bitsInBlock - nbits);
  static constexpr unsigned fBlocks =
      1ull + ((fbits - 1ull) / bitsInBlock); // nr of blocks with fraction bits
  static constexpr unsigned FSU =
      fBlocks - 1u; // FSU = Fraction Significant Unit: the index of the block
                    // that contains the most significant fraction bits
  static constexpr bt FSU_MASK = (ALL_ONES >> (fBlocks * bitsInBlock - fbits));
  static constexpr unsigned bitsInFSU =
      bitsInBlock - (fBlocks * bitsInBlock - fbits);

  static constexpr bt SIGN_BIT_MASK =
      bt(bt(1ull) << ((nbits - 1ull) % bitsInBlock));
  static constexpr bt LSB_BIT_MASK = bt(1ull);
  static constexpr bool MSU_CAPTURES_EXP = (1ull + es) <= bitsInMSU;
  static constexpr unsigned EXP_SHIFT =
      (MSU_CAPTURES_EXP
           ? (1 == nrBlocks ? (nbits - 1ull - es) : (bitsInMSU - 1ull - es))
           : 0);
  static constexpr bt MSU_EXP_MASK =
      ((ALL_ONES << EXP_SHIFT) & ~SIGN_BIT_MASK) & MSU_MASK;
  ...
  static constexpr int MIN_EXP_SUBNORMAL =
      1 - EXP_BIAS - int(fbits); // the scale of smallest ULP
  ...
  cfloat(const cfloat<nnbits, ees, bt, hasSubnormals, hasSupernormals,
                      isSaturating> &rhs) noexcept
      : _block{} {
    //		static_assert(nnbits < 64, "converting constructor marshalls
    // values through native double precision, and rhs has more bits");
    *this = double(rhs);
  }

  // converting constructors
  constexpr cfloat(const std::string &stringRep) : _block{} {
    assign(stringRep);
  }
  ...
  constexpr cfloat &operator=(signed char rhs) noexcept {
    return convert_signed_integer(rhs);
  }
  constexpr cfloat &operator=(short rhs) noexcept {
    return convert_signed_integer(rhs);
  }
  constexpr cfloat &operator=(int rhs) noexcept {
    return convert_signed_integer(rhs);
  }
  constexpr cfloat &operator=(long rhs) noexcept {
    return convert_signed_integer(rhs);
  }
  constexpr cfloat &operator=(long long rhs) noexcept {
    return convert_signed_integer(rhs);
  }

  constexpr cfloat &operator=(char rhs) noexcept {
    return convert_unsigned_integer(rhs);
  }
  constexpr cfloat &operator=(unsigned short rhs) noexcept {
    return convert_unsigned_integer(rhs);
  }
  constexpr cfloat &operator=(unsigned int rhs) noexcept {
    return convert_unsigned_integer(rhs);
  }
  constexpr cfloat &operator=(unsigned long rhs) noexcept {
    return convert_unsigned_integer(rhs);
  }
  constexpr cfloat &operator=(unsigned long long rhs) noexcept {
    return convert_unsigned_integer(rhs);
  }

  CONSTEXPRESSION cfloat &operator=(float rhs) noexcept {
    return convert_ieee754(rhs);
  }
  CONSTEXPRESSION cfloat &operator=(double rhs) noexcept {
    return convert_ieee754(rhs);
  }
  ...
  CONSTEXPRESSION cfloat &operator=(long double rhs) noexcept {
    return convert_ieee754(rhs);
  }
  explicit operator long double() const noexcept {
    return to_native<long double>();
  }
#endif
  ...
  cfloat &operator+=(double rhs) { return *this += cfloat(rhs); }
  cfloat &operator-=(const cfloat &rhs) {
    if constexpr (cfloat_trace_sub)
      std::cout << "---------------------- SUB -------------------"
                << std::endl;
    if (rhs.isnan())
      return *this += rhs;
  ...
  cfloat &operator/=(const cfloat &rhs) {
...
#if CFLOAT_THROW_ARITHMETIC_EXCEPTION
    if (rhs.iszero())
      throw cfloat_divide_by_zero();
    if (rhs.isnan())
      throw cfloat_divide_by_nan();
    if (isnan())
      throw cfloat_operand_is_nan();
#else
    if (isnan(NAN_TYPE_SIGNALLING) || rhs.isnan(NAN_TYPE_SIGNALLING)) {
    ...
    if (rhs.iszero()) {
      if (iszero()) {
        // zero divide by zero yields quiet NaN (in MSVC it is labeled -nan(ind)
        // for indeterminate)
        setnan(NAN_TYPE_QUIET);
      } else {
    ...
    if (isinf()) {
      if (rhs.isinf()) {
        // inf divide by inf yields quiet NaN (in MSVC it is labeled -nan(ind)
        // for indeterminate)
        setnan(NAN_TYPE_QUIET);
        return *this;
      } else {
  ...
  cfloat &operator++() {
    if constexpr (0 == nrBlocks) {
      ...
      if (sign()) {
        if (_block[MSU] ==
            (SIGN_BIT_MASK | 1ul)) { // pattern: 1.00.001 = minneg
        ...
        if constexpr (!hasSubnormals) {
          if (isdenormal()) {
            // special case, we need to jump past all the subnormal value
            // encodings which puts us on 0
            _block[MSU] = 0; // pattern: 0.00.000 = +0
          }
        }
      } else {
        ...
        if ((_block[MSU] & (MSU_MASK >> 1)) ==
            (MSU_MASK >> 1)) {          // pattern: 0.11.111 = nan
          _block[MSU] |= SIGN_BIT_MASK; // pattern: 1.11.111 = snan : wrap to
                                        // the other side of the encoding
        } else {
          ++_block[MSU];
        }
      }
      ...
      if (sign()) {
        // special case: pattern: 1.00.001 = minneg transitions to pattern:
        // 0.00.000 = +0
        if (isminnegencoding()) {
          setzero();
        } else {
        ...
        // special case: pattern: 0.11.111 = nan transitions to
        // pattern: 1.11.111 = snan
        if (isnanencoding()) {
          setnan(NAN_TYPE_SIGNALLING);
        } else {
          if constexpr (!hasSubnormals) {
            if (iszero()) {
              // special case, we need to jump past all the subnormal value
              // encodings minus 1 so that the increment code below ends up on
              // normal minpos
              setfraction(0xFFFF'FFFF'FFFF'FFFFull);
            }
          }
          ...
          for (unsigned i = 0; i < MSU; ++i) {
            if (carry) {
              if ((_block[i] & storageMask) ==
                  storageMask) { // block will overflow
                _block[i] = 0;
                carry = true;
              } else {
                ++_block[i];
  ...
  cfloat &operator--() {
    if constexpr (0 == nrBlocks) {
      ...
      if (sign()) {
        ...
        // special case: pattern: 0.00.000 = +0 transitions to pattern: 1.00.001
        // = minneg
        if (iszeroencoding()) {
          if constexpr (hasSubnormals) {
            setsign(true);
            ...
            // special case, we need to jump past all the subnormal value
            // encodings 1.01.0000 = minneg normal
  ...
  constexpr void setinf(bool sign = true) noexcept {
    if constexpr (0 == nrBlocks) {
      ...
      _block[MSU] = sign ? bt(MSU_MASK ^ LSB_BIT_MASK)
                         : bt(~SIGN_BIT_MASK & (MSU_MASK ^ LSB_BIT_MASK));
    } else if constexpr (2 == nrBlocks) {
  ...
  constexpr void setnan(int NaNType = NAN_TYPE_SIGNALLING) noexcept {
    ...
    _block[MSU] = NaNType == NAN_TYPE_SIGNALLING
                      ? MSU_MASK
                      : bt(~SIGN_BIT_MASK & MSU_MASK);
  }
  ...
  constexpr bool setexponent(int scale) {
    if (scale < MIN_EXP_SUBNORMAL || scale > MAX_EXP)
      return false; // this scale cannot be represented
  ...
  constexpr cfloat &setbits(uint64_t raw_bits) noexcept {
    ...
    _block[MSU] &= MSU_MASK; // enforce precondition for fast comparison by
  ...
  constexpr cfloat &maxpos() noexcept {
    if constexpr (hasSupernormals) {
      // maximum positive value has this bit pattern: 0-1...1-111...101, that
      // is, sign = 0, e = 11..11, f = 111...101
      clear();
      flip();
      setbit(nbits - 1ull, false); // sign = 0
      setbit(0ull, false);         // bit1 = 0
    } else {
      // maximum positive value has this bit pattern: 0-1...0-111...111, that
      // is, sign = 0, e = 11..10, f = 111...111
      clear();
      flip();
  ...
  constexpr cfloat &minpos() noexcept {
    if constexpr (hasSubnormals) {
      // minimum positive value has this bit pattern: 0-000-00...01, that is,
      // sign = 0, e = 000, f = 00001
      clear();
      setbit(0);
    } else {
      // minimum positive value has this bit pattern: 0-001-00...0, that is,
      // sign = 0, e = 001, f = 0000
      clear();
      setbit(fbits);
    }
  ...
  constexpr cfloat &minneg() noexcept {
    if constexpr (hasSubnormals) {
      // minimum negative value has this bit pattern: 1-000-00...01, that is,
      // sign = 1, e = 00, f = 00001
      clear();
      setbit(nbits - 1ull);
      setbit(0);
    } else {
      // minimum negative value has this bit pattern: 1-001-00...0, that is,
      // sign = 1, e = 001, f = 0000
      clear();
      setbit(fbits);
      setbit(nbits - 1ull);
  ...
  constexpr cfloat &maxneg() noexcept {
    if constexpr (hasSupernormals) {
      // maximum negative value has this bit pattern: 1-1...1-111...101, that
      // is, sign = 1, e = 1..1, f = 111...101
      clear();
      flip();
      setbit(0ull, false);
    } else {
      // maximum negative value has this bit pattern: 1-1...0-111...111, that
      // is, sign = 1, e = 11..10, f = 111...111
      clear();
      flip();
      setbit(fbits, false);
    }
  ...
  constexpr cfloat &flip() noexcept { // in-place one's complement
    ...
    _block[MSU] &=
        MSU_MASK; // assert precondition of properly nulled leading non-bits
    return *this;
  }
  ...
  /// </summary>
  /// <param name="stringRep">decimal scientific notation of a real number to be
  /// assigned</param> <returns>reference to this cfloat</returns> Clang doesn't
  /// support constexpr yet on string manipulations, so we need to make it
  /// conditional
  CONSTEXPRESSION cfloat &assign(const std::string &str) noexcept {
    ...
    if (nrChars > 2) {
      if (str[0] == '0' && str[1] == 'b') {
        ...
        std::cerr << "string must start with 0b: instead input pattern was "
                  << str << '\n';
        return *this;
      }
    } else {
    ...
    if (nrBits != nbits) {
      std::cerr << "number of bits in the string is " << nrBits
                << " and needs to be " << nbits << '\n';
      return *this;
    }
    if (nrDots != 2) {
      std::cerr << "number of segment delimiters in string is " << nrDots
                << " and needs to be 2 for a cfloat<>\n";
      return *this;
    }

    // assign the bits
    int field{0}; // three fields: sign, exponent, mantissa: fields are
                  // separated by a '.'
    ...
    for (unsigned i = 0; i < bits.size(); ++i) {
      ...
      if (c == '.') {
        ++field;
        if (field == 2) { // just finished parsing exponent field: we can now
                          // check the number of exponent bits
          if (nrExponentBits != es) {
            std::cerr
                << "provided binary string representation does not contain "
                << es << " exponent bits. Found " << nrExponentBits
                << ". Reset to 0\n";
            clear();
            return *this;
          }
    ...
    if (field != 2) {
      std::cerr << "provided binary string did not contain three fields "
                   "separated by '.': Reset to 0\n";
      clear();
      return *this;
    }
    return *this;
  ...
  constexpr bool sign() const noexcept {
    return (_block[MSU] & SIGN_BIT_MASK) == SIGN_BIT_MASK;
  }
  ...
  constexpr bool isinf(int InfType = INF_TYPE_EITHER) const noexcept {
    ...
    // the bit pattern encoding of Inf is independent of gradual overflow
    // (supernormal) configuration
    bool isNegInf = false;
    bool isPosInf = false;
    if constexpr (0 == nrBlocks) {
      return false;
      ...
      isPosInf = (_block[MSU] & MSU_MASK) ==
                 ((MSU_MASK ^ SIGN_BIT_MASK) ^ LSB_BIT_MASK);
    } else if constexpr (2 == nrBlocks) {
      bool isInf = (_block[0] == (BLOCK_MASK ^ LSB_BIT_MASK));
      isNegInf = isInf && ((_block[MSU] & MSU_MASK) == MSU_MASK);
      isPosInf =
          isInf && (_block[MSU] & MSU_MASK) == (MSU_MASK ^ SIGN_BIT_MASK);
    } else if constexpr (3 == nrBlocks) {
      bool isInf = (_block[0] == (BLOCK_MASK ^ LSB_BIT_MASK)) &&
                   (_block[1] == BLOCK_MASK);
      isNegInf = isInf && ((_block[MSU] & MSU_MASK) == MSU_MASK);
      ...
      bool isInf = (_block[0] == (BLOCK_MASK ^ LSB_BIT_MASK)) &&
                   (_block[1] == BLOCK_MASK) && (_block[2] == BLOCK_MASK);
      isNegInf = isInf && ((_block[MSU] & MSU_MASK) == MSU_MASK);
      isPosInf =
          isInf && (_block[MSU] & MSU_MASK) == (MSU_MASK ^ SIGN_BIT_MASK);
    } else {
    ...
    return (InfType == INF_TYPE_EITHER
                ? (isNegInf || isPosInf)
                : (InfType == INF_TYPE_NEGATIVE
                       ? isNegInf
  ...
  constexpr bool isnan(int NaNType = NAN_TYPE_EITHER) const noexcept {
    if constexpr (hasSupernormals) {
      ...
      if (issupernormal()) {
        // all these supernormal encodings are NANs, except for the encoding
        // representing INF
        bool isNaN = isinf() ? false : true;
        bool isNegNaN = isNaN && sign();
        bool isPosNaN = isNaN && !sign();
        return (NaNType == NAN_TYPE_EITHER
                    ? (isNaN)
                    : (NaNType == NAN_TYPE_SIGNALLING
                           ? isNegNaN
  ...
  // iszeroencoding returns true if it finds a pure -0 or +0 pattern and returns
  // false otherwise
  constexpr bool iszeroencoding() const noexcept {
    if constexpr (0 == nrBlocks) {
      ...
      return (_block[0] == 0) && _block[1] == 0 &&
             (_block[MSU] & ~SIGN_BIT_MASK) == 0;
    } else if constexpr (4 == nrBlocks) {
      return (_block[0] == 0) && _block[1] == 0 && _block[2] == 0 &&
             (_block[MSU] & ~SIGN_BIT_MASK) == 0;
    } else {
      for (unsigned i = 0; i < nrBlocks - 1; ++i)
        if (_block[i] != 0)
  ...
  // isminnegencoding returns true if it find the pattern 1.00.00001 and returns
  // false otherwise
  constexpr bool isminnegencoding() const noexcept {
    if constexpr (0 == nrBlocks) {
      ...
      return ((_block[0] == 1ul) && (_block[1] == 0) &&
              (_block[2] == SIGN_BIT_MASK));
    } else if constexpr (4 == nrBlocks) {
      return ((_block[0] == 1ul) && (_block[1] == 0) && (_block[2] == 0) &&
              (_block[3] == SIGN_BIT_MASK));
    } else {
      if (_block[0] != 1ul)
        return false;
      for (unsigned i = 1; i < nrBlocks - 2; ++i)
        if (_block[i] != 0)
          return false;
  ...
  constexpr bool isnanencoding(int NaNType = NAN_TYPE_EITHER) const noexcept {
    // the bit encoding of NaN is independent of the gradual overflow
    // configuration
    bool isNaN = true;
    bool isNegNaN = false;
    bool isPosNaN = false;
  ...
  // isnormal returns true if 0 or exponent bits are not all zero or one, false
  // otherwise
  constexpr bool isnormal() const noexcept {
    if (iszeroencoding())
      return true; // filter out the one special case
    blockbinary<es, bt> e{};
    exponent(e);
  ...
  constexpr bool isdenormal() const noexcept {
    if (iszeroencoding())
      return false; // filter out the one special case
    blockbinary<es, bt> e{};
    exponent(e);
    return e.iszero();
  }
  ...
  constexpr bool isinteger() const noexcept {
    return false;
  } // return (floor(*this) == *this) ? true : false; }
  ...
  constexpr bool inrange(NativeReal v) const noexcept {
    ...
    if (v > 0) {
      ...
      cfloat<nbits + 1, es, BlockType, hasSubnormals, hasSupernormals,
             isSaturating>
          d{};
      d = NativeReal(c);
      ++d;
  ...
  constexpr void sign(bool &s) const { s = sign(); }
    ...
    if constexpr (0 == nrBlocks)
      ...
      if (MSU_CAPTURES_EXP) {
        ...
        for (unsigned i = 0; i < es; ++i) {
          e.setbit(i, at(nbits - 1ull - es + i));
        }
      }
    }
  }
  template <unsigned targetFractionBits>
  constexpr blockbinary<targetFractionBits, bt> &
  fraction(blockbinary<targetFractionBits, bt> &f) const {
    static_assert(
        targetFractionBits >= fbits,
        "target blockbinary is too small and can't receive all fraction bits");
    f.clear();
    if constexpr (0 == nrBlocks)
      return f;
    else if constexpr (1 == nrBlocks) {
  ...
  constexpr uint64_t fraction_ull() const {
    uint64_t raw{0};
    if constexpr (nbits - es - 1ull <
                  65ull) { // no-op if precondition doesn't hold
      if constexpr (1 == nrBlocks) {
        ...
        uint64_t fbitMask = 0xFFFF'FFFF'FFFF'FFFF >> (64 - fbits);
        raw = fbitMask &
              ((uint64_t(_block[1]) << bitsInBlock) | uint64_t(_block[0]));
      } else if constexpr (3 == nrBlocks) {
        uint64_t fbitMask = 0xFFFF'FFFF'FFFF'FFFF >> (64 - fbits);
        raw = fbitMask &
              ((uint64_t(_block[2]) << (2 * bitsInBlock)) |
               (uint64_t(_block[1]) << bitsInBlock) | uint64_t(_block[0]));
      } else if constexpr (4 == nrBlocks) {
        uint64_t fbitMask = 0xFFFF'FFFF'FFFF'FFFF >> (64 - fbits);
        raw = fbitMask &
              ((uint64_t(_block[3]) << (3 * bitsInBlock)) |
               (uint64_t(_block[2]) << (2 * bitsInBlock)) |
               (uint64_t(_block[1]) << bitsInBlock) | uint64_t(_block[0]));
      } else {
        uint64_t mask{1};
  ...
  constexpr unsigned significant(blockbinary<fhbits, bt> &s,
                                 bool isNormal = true) const {
    unsigned shift = 0;
    if (iszero())
      return 0;
    if constexpr (0 == nrBlocks)
      ...
      if (isNormal) {
        ...
        //:
        //"
        //<< fhbits << " : " << to_binary(significant, true) << std::endl;
        shift = fhbits - msb;
        significant <<= shift;
      }
      ...
      s.clear();
      // TODO: design and implement a block-oriented algorithm, this sequential
      // algorithm is super slow
      if (isNormal) {
        s.setbit(fbits);
        for (unsigned i = 0; i < fbits; ++i) {
          s.setbit(i, at(i));
        ...
        unsigned msb = 0;
        for (unsigned i = 0; i < fbits;
             ++i) { // msb protected from not being assigned through iszero test
                    // at prelude of function
          msb = fbits - 1ull - i;
          if (test(msb))
            break;
        }
  ...
  // precision to compute, which means that all sub-values need to be
  // representable by the native precision. A more accurate approximation would
  // require an adaptive precision algorithm with a final rounding step.
  template <typename TargetFloat> TargetFloat to_native() const {
    TargetFloat v{0.0};
    if (iszero()) {
      // the optimizer might destroy the sign
      return sign() ? -TargetFloat(0) : TargetFloat(0);
    } else if (isnan()) {
      v = sign() ? std::numeric_limits<TargetFloat>::signaling_NaN()
                 : std::numeric_limits<TargetFloat>::quiet_NaN();
    } else if (isinf()) {
      v = sign() ? -std::numeric_limits<TargetFloat>::infinity()
                 : std::numeric_limits<TargetFloat>::infinity();
    } else { // TODO: this approach has catastrophic cancellation when nbits is
             // large and native target float is too small
      TargetFloat f{0};
      ...
      if constexpr (hasSubnormals) {
        if (ebits.iszero()) {
          // subnormals: (-1)^s * 2^(2-2^(es-1)) * (f/2^fbits))
          TargetFloat exponentiation = TargetFloat(
              subnormal_exponent[es]); // precomputed values for 2^(2-2^(es-1))
      ...
      if constexpr (hasSupernormals) {
        ...
        if (-64 < exponent && exponent < 64) {
          TargetFloat exponentiation =
              (exponent >= 0 ? TargetFloat(1ull << exponent)
  ...
  // we are using the same block type so that we can use block copies to move
  // bits around. Since we tend to have at least two exponent bits, this will
  // lead to most cfloat<->blocktriple cases being efficient as the block types
  // are aligned. The relationship between the source cfloat and target
  // blocktriple is not arbitrary, enforce it by setting the blocktriple fbits
  // to the cfloat's (nbits - es - 1)
  constexpr void
  normalize(blocktriple<fbits, BlockTripleOperator::REP, bt> &tgt) const {
    // test special cases
    if (isnan()) {
      tgt.setnan();
    } else if (isinf()) {
      ...
      // so that normalize can be used to generate blocktriples for
      // add/sub/mul/div/sqrt
      if (isnormal()) {
        if constexpr (fbits < 64) { // max 63 bits of fraction to yield 64bit of
                                    // raw significant bits
  ...
  // normalize a cfloat to a blocktriple used in add/sub, which has the form
  // 00h.fffff that is 3 + fbits, the 3 extra bits are required to be able to
  // use 2's complement and capture the largest value of an
  // addition/subtraction.
  // TODO: currently abits = 2*fhbits as the worst case input argument size to
  // capture the smallest normal value in aligned form. There is a
  // faster/smaller implementation where the input is constrainted to just the
  // round, guard, and sticky bits.
  constexpr void normalizeAddition(
      blocktriple<fbits, BlockTripleOperator::ADD, bt> &tgt) const {
    using BlockTripleConfiguration =
        blocktriple<fbits, BlockTripleOperator::ADD, bt>;
    // test special cases
    if (isnan()) {
      tgt.setnan();
    } else if (isinf()) {
      ...
      if (isnormal()) {
        if constexpr (fbits < 64 &&
                      BlockTripleConfiguration::rbits < (64 - fbits)) {
          uint64_t raw = fraction_ull();
          raw |= (1ull << fbits); // add the hidden bit
          // std::cout << "normalize      : " << *this << '\n';
          // std::cout << "significant    : " << to_binary(raw, fbits + 2) <<
          // '\n';
          raw <<= BlockTripleConfiguration::rbits; // rounding bits required for
                                                   // correct rounding
          // std::cout << "rounding shift : " << to_binary(raw, fbits + 2 +
          // BlockTripleConfiguration::rbits) << '\n';
          tgt.setbits(raw);
        } else {
          blockcopy(tgt);
          tgt.setradix();
          ...
          tgt.bitShift(
              BlockTripleConfiguration::rbits); // rounding bits required for
                                                // correct rounding
        }
      } else {
        if (isdenormal()) { // it is a subnormal encoding in this target cfloat
          if constexpr (hasSubnormals) {
            if constexpr (BlockTripleConfiguration::rbits < (64 - fbits)) {
              ...
              int shift = MIN_EXP_NORMAL - scale;
              raw <<=
                  shift; // shift but do NOT add a hidden bit as the MSB of the
                         // subnormal is shifted in the hidden bit position
              raw <<= BlockTripleConfiguration::rbits; // rounding bits required
              ...
              tgt.bitShift(
                  shift +
                  BlockTripleConfiguration::rbits); // rounding bits required
                                                    // for correct rounding
            }
          ...
          // by design, a cfloat is either normal, subnormal, or supernormal, so
          // this else clause is by deduction covering a supernormal
          if constexpr (hasSupernormals) {
            if constexpr (fbits < 64 &&
                          BlockTripleConfiguration::rbits < (64 - fbits)) {
              uint64_t raw = fraction_ull();
          ...
          } else { // this cfloat has no supernormals and thus this represents a
                   // nan, signalling or quiet determined by the sign
            tgt.setnan(tgt.sign());
          }
        }
      }
  ...
  // Normalize a cfloat to a blocktriple used in mul, which has the form
  // 0'00001.fffff that is 2*fbits, plus 1 overflow bit, and the radix set at
  // <fbits>. The result radix will go to 2*fbits after multiplication.
  constexpr void normalizeMultiplication(
      blocktriple<fbits, BlockTripleOperator::MUL, bt> &tgt) const {
    // test special cases
    if (isnan()) {
      tgt.setnan();
    } else if (isinf()) {
    ...
    tgt.setradix(fbits); // override the radix with the input scale for accurate
                         // value printing
  }

  // normalize a cfloat to a blocktriple used in div, which has the form
  // 0'00000'00001.fffff that is 3*fbits, plus 1 overflow bit, and the radix set
  // at <fbits>. the result radix will go to 2*fbits after multiplication.
  // TODO: needs implementation
  constexpr void normalizeDivision(
      blocktriple<fbits, BlockTripleOperator::DIV, bt> &tgt) const {
    constexpr unsigned divshift =
        blocktriple<fbits, BlockTripleOperator::DIV, bt>::divshift;
    // test special cases
    if (isnan()) {
      tgt.setnan();
    } else if (isinf()) {
  ...
  void constexprClassParameters() const noexcept {
    ...
    std::cout << "LSB_BIT_MASK      : " << to_binary(LSB_BIT_MASK, 0, true)
              << '\n';
    std::cout << "MSU CAPTURES_EXP  : "
              << (MSU_CAPTURES_EXP ? "yes\n" : "no\n");
...
protected:
  ...
  // TODO: this method does not protect against being called with a signed
  // integer
  ...
  constexpr cfloat &convert_unsigned_integer(const Ty &rhs) noexcept {
    ...
    int msb = static_cast<int>(findMostSignificantBit(raw)) -
              1; // msb > 0 due to zero test above
    int exponent = msb;
    // remove the MSB as it represents the hidden bit in the cfloat
    // representation
    ...
    if constexpr (fbits < (64 - es)) {
      uint64_t biasedExponent = static_cast<uint64_t>(
          static_cast<long long>(exponent) + static_cast<long long>(EXP_BIAS));
      uint64_t bits = 0;
      bits <<= es;
      bits |= biasedExponent;
      bits <<= fbits;
...
public:
  ...
  CONSTEXPRESSION cfloat &convert_ieee754(Real rhs) noexcept {
    if constexpr (nbits == 32 && es == 8 && sizeof(Real) == 4) {
      // we CANNOT use the native conversion to float as cfloats have
      // supernormals which IEEE-754 does not have and thus a native conversion
      // would destroy only if the Real type is a float can we use the direct
      // conversion
      ...
      if (rawExponent ==
          ieee754_parameter<Real>::eallset) { // nan and inf need to be remapped
        if (rawFraction == (ieee754_parameter<Real>::fmask &
                            ieee754_parameter<Real>::snanmask) ||
            rawFraction == (ieee754_parameter<Real>::fmask &
                            (ieee754_parameter<Real>::qnanmask |
                             ieee754_parameter<Real>::snanmask))) {
          ...
          setnan(NAN_TYPE_SIGNALLING);
          // setsign(s);  a cfloat encodes a signalling nan with sign = 1, and a
          // quiet nan with sign = 0
          return *this;
        }
        if (rawFraction == (ieee754_parameter<Real>::fmask &
                            ieee754_parameter<Real>::qnanmask)) {
      ...
      int exponent = static_cast<int>(rawExponent) -
                     ieee754_parameter<Real>::bias; // unbias the exponent
      ...
      //
      // One problem here is that at the rounding cusps of maxpos <-> inf <->
      // nan you need to go through the rounding logic to know which encoding
      // you end up with. For each specific cfloat configuration, you can work
      // out these rounding cusps but they need to go through the value
      // transformation to map them back to native IEEE-754. That is a complex
      // computation to do as a static constexpr as you need to construct the
      // value, then evaluate it, and store it.
      //
      // The algorithm used here is to check for the obvious out of range values
      // by comparing their scale to the max scale this cfloat encoding can
      // represent. For the rounding cusps, we go through the rounding logic,
      // and then clean up after rounding using the observation that no
      // conversion from a value can ever yield the NaN encoding.
      //
      // The rounding logic will correctly sort between maxpos and inf, and we
      // clean up any NaN encodings by projecting back to the configuration's
      // saturation rule.
      //
      // We could improve on this by creating the database of rounding cusps and
      // referencing them with a direct value comparison with the input. That
      // would be the most performant implementation.
      if (exponent > MAX_EXP) {
        if constexpr (isSaturating) {
          if (s)
            this->maxneg();
          else
            this->maxpos(); // saturate to maxpos or maxneg
        } else {
          if constexpr (hasSupernormals) {
            setnan();
          } else {
      ...
      if constexpr (hasSubnormals) {
        if (exponent < MIN_EXP_SUBNORMAL - 1) {
          // map to +-0 any values that have a scale less than
          // (MIN_EXP_SUBMORNAL - 1)
        ...
        if (exponent < MIN_EXP_NORMAL - 1) {
          // map to +-0 any values that have a scale less than (MIN_EXP_MORNAL -
          // 1)
          this->setbit(nbits - 1, s);
          return *this;
        }
      ...
      std::cout << "exponent bits     : "
                << to_binary(rawExponent, ieee754_parameter<Real>::ebits, true)
                << '\n';
      std::cout << "fraction bits     : "
                << to_binary(rawFraction, ieee754_parameter<Real>::fbits, true)
                << '\n';
      std::cout << "exponent value    : " << exponent << '\n';
#endif

      // do the following scenarios have different rounding bits?
      // input is normal, cfloat is normal           <-- rounding can happen
      // with native ieee-754 bits input is normal, cfloat is subnormal input is
      // subnormal, cfloat is normal input is subnormal, cfloat is subnormal
      ...
      if constexpr (fbits < ieee754_parameter<Real>::fbits) {
        // this is the common case for cfloats that are smaller in size compared
        // to single and double precision IEEE-754
        constexpr int rightShift =
            ieee754_parameter<Real>::fbits -
            fbits; // this is the bit shift to get the MSB of the src to the MSB
                   // of the tgt
        uint32_t biasedExponent{0};
        int adjustment{
            0}; // right shift adjustment for subnormal representation
        uint64_t mask;
        if (rawExponent != 0) {
          ...
          //(MIN_EXP_SUBNORMAL
          //- 1)
          //&& exponent < MIN_EXP_NORMAL) {
          if (exponent < MIN_EXP_NORMAL) {
            //						exponent = (exponent <
            // MIN_EXP_SUBNORMAL ? MIN_EXP_SUBNORMAL : exponent); // clip to the
            // smallest subnormal
            // exponent, otherwise the adjustment is off
            // the value is a subnormal number in this representation:
            // biasedExponent = 0 add the hidden bit to the fraction bits so the
            // denormalization has the correct MSB
            rawFraction |= ieee754_parameter<Real>::hmask;

            // fraction processing: we have 1 hidden + 23 explicit fraction bits
            // f = 1.ffff 2^exponent * 2^fbits * 2^-(2-2^(es-1)) = 1.ff...ff >>
            // (23 - (-exponent + fbits - (2 -2^(es-1)))) -exponent because we
            // are right shifting and exponent in this range is negative
            adjustment = -(exponent + subnormal_reciprocal_shift[es]);
            // this is the right shift adjustment required for subnormal
            // representation due to the scale of the input number, i.e. the
            // exponent of 2^-adjustment
            ...
            biasedExponent = static_cast<uint32_t>(
                exponent + EXP_BIAS); // project the exponent into the target
            // fraction processing
            // float structure is: seee'eeee'efff'ffff'ffff'ffff'ffff'ffff, s =
            // sign, e - exponent bit, f = fraction bit target structure is for
            // example cfloat<8,2>: seef'ffff since both are normals, we can
            // shift the incoming fraction to the target structure bits, and
            // round MSB of source = 23 - 1, MSB of target = fbits - 1: shift =
            // MSB of src - MSB of tgt => 23 - fbits
            adjustment = 0;
          }
          if constexpr (rightShift > 0) {
...
#if TRACE_CONVERSION
            std::cout << "fraction bits     : "
                      << to_binary(rawFraction, ieee754_parameter<Real>::nbits,
                                   true)
                      << '\n';
            std::cout << "lsb mask bits     : "
                      << to_binary(mask, ieee754_parameter<Real>::nbits, true)
                      << '\n';
#endif
            mask =
                (1ull << (rightShift + adjustment)); // bit mask for the lsb bit
            ...
            if ((rightShift + adjustment) > 1) {
              mask =
                  (0xFFFF'FFFF'FFFF'FFFFull << (rightShift + adjustment - 2));
              mask = ~mask;
            } else {
              mask = 0;
            }
            ...
            std::cout << "fraction bits     : "
                      << to_binary(rawFraction, ieee754_parameter<Real>::nbits,
                                   true)
                      << '\n';
            std::cout << "sticky mask bits  : "
                      << to_binary(mask, ieee754_parameter<Real>::nbits, true)
                      << '\n';
#endif
            bool sticky = (mask & rawFraction);
            rawFraction >>= (static_cast<int64_t>(rightShift) +
                             static_cast<int64_t>(adjustment));

            // execute rounding operation
            if (guard) {
              if (lsb && (!round && !sticky))
                ++rawFraction; // round to even
              if (round || sticky)
                ++rawFraction;
              if (rawFraction == (1ull << fbits)) { // overflow
                if (biasedExponent ==
                    ALL_ONES_ES) { // overflow to INF == .111..01
            ...
            std::cout << "rounding decision : "
                      << (lsb && (!round && !sticky) ? "round to even\n"
                                                     : "-\n");
            std::cout << "rounding direction: "
                      << (round || sticky ? "round up\n" : "round down\n");
#endif
          } else { // all bits of the float go into this representation and need
                   // to be shifted up, no rounding necessary
            int shiftLeft = fbits - ieee754_parameter<Real>::fbits;
            rawFraction <<= shiftLeft;
          }
#if TRACE_CONVERSION
          std::cout << "biased exponent   : " << biasedExponent << " : 0x"
                    << std::hex << biasedExponent << std::dec << '\n';
          std::cout << "right shift       : " << rightShift << '\n';
          std::cout << "adjustment shift  : " << adjustment << '\n';
          std::cout << "sticky bit mask   : " << to_binary(mask, 32, true)
                    << '\n';
          std::cout << "fraction bits     : "
                    << to_binary(rawFraction, 32, true) << '\n';
#endif
          // construct the target cfloat
          uint64_t bits = (s ? 1ull : 0ull);
          bits <<= es;
          ...
          mask = 0x00FF'FFFFu >>
                 (fbits + exponent + subnormal_reciprocal_shift[es] +
                  1); // mask for sticky bit

          // fraction processing: we have fbits+1 bits = 1 hidden + fbits
          // explicit fraction bits f = 1.ffff  2^exponent * 2^fbits *
          // 2^-(2-2^(es-1)) = 1.ff...ff >> (23 - (-exponent + fbits - (2
          // -2^(es-1)))) -exponent because we are right shifting and exponent
          // in this range is negative
          adjustment = -(
              exponent +
              subnormal_reciprocal_shift[es]); // this is the right shift
                                               // adjustment due to the scale of
                                               // the input number, i.e. the
                                               // exponent of 2^-adjustment
#if TRACE_CONVERSION
        ...
        uint64_t biasedExponent =
            static_cast<uint64_t>(static_cast<int64_t>(exponent) + EXP_BIAS);
        ...
        if constexpr (nbits < 65) {
          ...
          // reference example: nbits = 40, es = 8, fbits = 31: rhs = float
          // fbits = 23; shift left by (31 - 23) = 8
          ...
          if constexpr (bitsInBlock < 64) {
            if (rawExponent != 0) {
              // reference example: nbits = 128, es = 15, fbits = 112: rhs =
              // float: shift left by (112 - 23) = 89
              setbits(biasedExponent);
              shiftLeft(fbits);
              bt fractionBlock[nrBlocks]{0};
              // copy fraction bits
              unsigned blocksRequired =
                  (8 * sizeof(rawFraction) + 1) / bitsInBlock;
              unsigned maxBlockNr =
                  (blocksRequired < nrBlocks ? blocksRequired : nrBlocks);
              uint64_t mask =
                  static_cast<uint64_t>(ALL_ONES); // set up the block mask
              unsigned shift = 0;
              ...
              if (bitsToShift > 0) {
                // construct the mask for the upper bits in the block that need
                // to move to the higher word
                bt bitsToMoveMask = bt(ALL_ONES << (bitsInBlock - bitsToShift));
                for (unsigned i = MSU; i > 0; --i) {
                  fractionBlock[i] <<= bitsToShift;
                  // mix in the bits from the right
                  bt bits = static_cast<bt>(
                      bitsToMoveMask &
                      fractionBlock[i - 1]); // operator & yields an int
              ...
              // enforce precondition for fast comparison by properly nulling
              // bits that are outside of nbits
              _block[MSU] &= MSU_MASK;
      ...
      // post-processing results to implement saturation and projection after
      // rounding logic
      if constexpr (isSaturating) {
  ...
  // post-processing results to implement saturation and projection after
  // rounding logic arithmetic bit operations can't produce NaN encodings, so we
  // need to re-interpret these encodings and 'project' them to the proper
  // values.
...
protected:
  ...
  constexpr uint64_t round(StorageType raw, int &exponent) noexcept {
    if constexpr (fhbits < srcbits) {
      ...
      if constexpr (shift < (sizeof(StorageType) * 8)) {
        ...
        std::cerr << "round: shift " << shift << " >= " << sizeof(StorageType)
                  << std::endl;
        raw = 0;
      }
    }
  ...
  void shiftLeft(int leftShift) {
    if (leftShift == 0)
      return;
    if (leftShift < 0)
      return shiftRight(-leftShift);
    if (leftShift > long(nbits))
      leftShift = nbits; // clip to max
    ...
    // construct the mask for the upper bits in the block that need to move to
    // the higher word
    //		bt mask = static_cast<bt>(0xFFFFFFFFFFFFFFFFull << (bitsInBlock
    //- leftShift));
    bt mask = ALL_ONES;
  ...
  void shiftRight(int rightShift) {
    ...
    bt mask = ALL_ONES;
    mask >>=
        (bitsInBlock - rightShift); // this is a mask for the lower bits in the
                                    // block that need to move to the lower word
    for (unsigned i = 0; i < MSU;
         ++i) { // TODO: can this be improved? we should not have to work on the
                // upper blocks in case we block shifted
      _block[i] >>= rightShift;
      // mix in the bits from the left
      bt bits =
          static_cast<bt>(mask & _block[i + 1]); // & operator returns an int
    ...
    // enforce precondition for fast comparison by properly nulling bits that
    // are outside of nbits
    _block[MSU] &= MSU_MASK;
  }
  ...
  template <typename T> constexpr void blockcopy(T &tgt) const {
    // brute force copy of blocks
...
private:
  ...
  // template parameters need names different from class template parameters
  // (for gcc and clang)
  template <unsigned nnbits, unsigned nes, typename nbt, bool nsub, bool nsup,
            bool nsat>
  friend std::ostream &
  operator<<(std::ostream &ostr,
             const cfloat<nnbits, nes, nbt, nsub, nsup, nsat> &r);
  template <unsigned nnbits, unsigned nes, typename nbt, bool nsub, bool nsup,
            bool nsat>
  friend std::istream &
  operator>>(std::istream &istr, cfloat<nnbits, nes, nbt, nsub, nsup, nsat> &r);

  template <unsigned nnbits, unsigned nes, typename nbt, bool nsub, bool nsup,
            bool nsat>
  friend bool operator==(const cfloat<nnbits, nes, nbt, nsub, nsup, nsat> &lhs,
...
template <unsigned nbits, unsigned es, typename bt, bool hasSubnormals,
          bool hasSupernormals, bool isSaturating>
std::string
to_decimal_fixpnt_string(const cfloat<nbits, es, bt, hasSubnormals,
                                      hasSupernormals, isSaturating> &value,
                         long long precision) {
  constexpr unsigned fbits = cfloat<nbits, es, bt, hasSubnormals,
                                    hasSupernormals, isSaturating>::fbits;
  constexpr unsigned bias = cfloat<nbits, es, bt, hasSubnormals,
                                   hasSupernormals, isSaturating>::EXP_BIAS;
  ...
  discretizationLevels.powerOf2(
      fbits); // calculate the discretization levels of this range
  ...
  int nrLeadingZeros =
      static_cast<int>(range.size()) - static_cast<int>(partial.size()) - 1;
  if (nrLeadingZeros >= 0)
    str << "0.";
  for (int i = 0; i < nrLeadingZeros; ++i)
    str << '0';
  int digitsWritten = (nrLeadingZeros > 0) ? nrLeadingZeros : 0;
  int position = static_cast<int>(partial.size()) - 1;
  for (support::decimal::const_reverse_iterator rit = partial.rbegin();
       rit != partial.rend(); ++rit) {
    str << (int)*rit;
    ++digitsWritten;
    if (position == fbits)
      str << '.';
...
template <unsigned nbits, unsigned es, typename bt, bool hasSubnormals,
          bool hasSupernormals, bool isSaturating>
std::string to_string(const cfloat<nbits, es, bt, hasSubnormals,
                                   hasSupernormals, isSaturating> &value,
                      long long precision) {
  constexpr unsigned fbits = cfloat<nbits, es, bt, hasSubnormals,
                                    hasSupernormals, isSaturating>::fbits;
...
template <unsigned nbits, unsigned es, typename bt, bool hasSubnormals,
          bool hasSupernormals, bool isSaturating>
inline std::ostream &
operator<<(std::ostream &ostr, const cfloat<nbits, es, bt, hasSubnormals,
                                            hasSupernormals, isSaturating> &v) {
  ...
  bool scientific =
      (ff & std::ios_base::scientific) == std::ios_base::scientific;
  ...
  std::streamsize repWidth =
      static_cast<std::streamsize>(representation.size());
  if (width > repWidth) {
    std::streamsize diff =
        width - static_cast<std::streamsize>(representation.size());
    char fill = ostr.fill();
...
template <unsigned nbits, unsigned es, typename bt, bool hasSubnormals,
          bool hasSupernormals, bool isSaturating>
inline std::istream &
operator>>(std::istream &istr, const cfloat<nbits, es, bt, hasSubnormals,
                                            hasSupernormals, isSaturating> &v) {
  istr >> v._fraction;
  return istr;
}
...
ulp(const cfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>
        &a) {
...
template <unsigned nbits, unsigned es, typename bt, bool hasSubnormals,
          bool hasSupernormals, bool isSaturating>
inline std::string
to_binary(const cfloat<nbits, es, bt, hasSubnormals, hasSupernormals,
                       isSaturating> &number,
...
template <unsigned nbits, unsigned es, typename bt, bool hasSubnormals,
          bool hasSupernormals, bool isSaturating>
inline std::string
to_triple(const cfloat<nbits, es, bt, hasSubnormals, hasSupernormals,
                       isSaturating> &number,
          bool nibbleMarker = true) {
  std::stringstream s;
  blocktriple<cfloat<nbits, es, bt, hasSubnormals, hasSupernormals,
                     isSaturating>::fbits,
              BlockTripleOperator::REP, bt>
      triple;
...
// convenience method to gain access to the values of the constexpr variables
// that govern the cfloat behavior
template <unsigned nbits, unsigned es, typename bt = uint8_t,
          bool hasSubnormals = false, bool hasSupernormals = false,
...
template <unsigned nnbits, unsigned nes, typename nbt, bool nsub, bool nsup,
          bool nsat>
inline bool operator==(const cfloat<nnbits, nes, nbt, nsub, nsup, nsat> &lhs,
                       const cfloat<nnbits, nes, nbt, nsub, nsup, nsat> &rhs) {
...
inline bool operator<(const cfloat<nnbits, nes, nbt, nsub, nsup, nsat> &lhs,
  ...
  if (lhs.isinf(INF_TYPE_NEGATIVE) && rhs.isinf(INF_TYPE_NEGATIVE))
    return false;
  if (lhs.isinf(INF_TYPE_POSITIVE) && rhs.isinf(INF_TYPE_POSITIVE))
    return false;
  if constexpr (nsub) {
    cfloat<nnbits, nes, nbt, nsub, nsup, nsat> diff = (lhs - rhs);
    return (!diff.iszero() && diff.sign()) ? true
                                           : false; // got to guard against -0
  } else {
    if (lhs.iszero() && rhs.iszero())
      return false; // we need to 'collapse' all zero encodings
    ...
    if (lhs.scale() == rhs.scale()) {
      // compare fractions: we do not have subnormals, so we can ignore the
      // hidden bit
      blockbinary<nnbits - 1ull - nes, nbt> l, r;
      lhs.fraction(l);
      rhs.fraction(r);
      blockbinary<nnbits - nes, nbt> ll,
          rr; // fbits + 1 so we can 0 extend to honor 2's complement encoding
              // of blockbinary
...
template <unsigned nbits, unsigned es, typename bt, bool hasSubnormals,
          bool hasSupernormals, bool isSaturating>
inline cfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>
operator+(const cfloat<nbits, es, bt, hasSubnormals, hasSupernormals,
                       isSaturating> &lhs,
          const cfloat<nbits, es, bt, hasSubnormals, hasSupernormals,
                       isSaturating> &rhs) {
...
operator/(const cfloat<nbits, es, bt, hasSubnormals, hasSupernormals,
          ...
          const cfloat<nbits, es, bt, hasSubnormals, hasSupernormals,
                       isSaturating> &rhs) {
  cfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating> ratio(
      lhs);
  ratio /= rhs;
  return ratio;
}
...
template <unsigned nbits, unsigned es, typename bt, bool hasSubnormals,
          bool hasSupernormals, bool isSaturating>
inline cfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>
operator+(float lhs, const cfloat<nbits, es, bt, hasSubnormals, hasSupernormals,
                                  isSaturating> &rhs) {
  cfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating> sum(lhs);
  sum += rhs;
  return sum;
...
template <unsigned nbits, unsigned es, typename bt, bool hasSubnormals,
          bool hasSupernormals, bool isSaturating>
inline cfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>
operator+(double lhs, const cfloat<nbits, es, bt, hasSubnormals,
                                   hasSupernormals, isSaturating> &rhs) {
  cfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating> sum(lhs);
  sum += rhs;
  return sum;
}
...
template <unsigned nbits, unsigned es, typename bt, bool hasSubnormals,
          bool hasSupernormals, bool isSaturating>
inline cfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>
operator+(int lhs, const cfloat<nbits, es, bt, hasSubnormals, hasSupernormals,
                                isSaturating> &rhs) {
  cfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating> sum(lhs);
  sum += rhs;
  return sum;
...
template <unsigned nbits, unsigned es, typename bt, bool hasSubnormals,
          bool hasSupernormals, bool isSaturating>
inline cfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>
operator+(unsigned int lhs, const cfloat<nbits, es, bt, hasSubnormals,
                                         hasSupernormals, isSaturating> &rhs) {
  cfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating> sum(lhs);
  sum += rhs;
  return sum;
...
template <unsigned nbits, unsigned es, typename bt, bool hasSubnormals,
          bool hasSupernormals, bool isSaturating>
inline cfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>
operator+(long long lhs, const cfloat<nbits, es, bt, hasSubnormals,
                                      hasSupernormals, isSaturating> &rhs) {
  cfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating> sum(lhs);
  sum += rhs;
  return sum;
...
template <unsigned nbits, unsigned es, typename bt, bool hasSubnormals,
          bool hasSupernormals, bool isSaturating>
inline cfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>
operator+(unsigned long long lhs,
          const cfloat<nbits, es, bt, hasSubnormals, hasSupernormals,
                       isSaturating> &rhs) {
  cfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating> sum(lhs);
  sum += rhs;
...
template <unsigned nbits, unsigned es, typename bt, bool hasSubnormals,
          bool hasSupernormals, bool isSaturating>
inline bool operator==(const cfloat<nbits, es, bt, hasSubnormals,
                                    hasSupernormals, isSaturating> &lhs,
                       float rhs) {
...
inline bool operator==(const cfloat<nbits, es, bt, hasSubnormals,
  ...
  return operator==(
      lhs,
      cfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>(rhs));
}
template <unsigned nbits, unsigned es, typename bt, bool hasSubnormals,
          bool hasSupernormals, bool isSaturating>
inline bool operator!=(const cfloat<nbits, es, bt, hasSubnormals,
...
inline bool operator<=(const cfloat<nbits, es, bt, hasSubnormals,
                                    hasSupernormals, isSaturating> &lhs,
                       int rhs) {
  return operator<(lhs, cfloat<nbits, es, bt, hasSubnormals, hasSupernormals,
                               isSaturating>(rhs)) ||
         operator==(lhs, cfloat<nbits, es, bt>(rhs));
}
template <unsigned nbits, unsigned es, typename bt, bool hasSubnormals,
          bool hasSupernormals, bool isSaturating>
inline bool operator>=(const cfloat<nbits, es, bt, hasSubnormals,
...
template <unsigned nbits, unsigned es, typename bt, bool hasSubnormals,
          bool hasSupernormals, bool isSaturating>
inline cfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>
frexp(const cfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>
          &x,
      int *exp) {
  *exp = x.scale();
  cfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating> fraction(
      x);
...
template <unsigned nbits, unsigned es, typename bt, bool hasSubnormals,
          bool hasSupernormals, bool isSaturating>
inline cfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>
ldexp(const cfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>
          &x,
      int exp) {
