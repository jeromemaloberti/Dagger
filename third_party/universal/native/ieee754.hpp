...
template <typename Real> class ieee754_parameter {
public:
  ...
  static constexpr uint64_t hfmask =
      0; // mask of the signficicant field, i.e. hidden bit + fraction bits
  static constexpr uint64_t fmsb =
      0; // mask of the most significant fraction bit
  ...
  static constexpr Real minSubnormal =
      Real(0.0); // value of the smallest subnormal value
  static constexpr int minNormalExp =
      0; // exponent value of smallest normal value
  static constexpr int minSubnormalExp =
      0; // exponent value of smallest subnormal value
};
...
std::ostream &operator<<(std::ostream &ostr, const ieee754_parameter<Real> &v) {
  ...
  ostr << "mask of exponent value      : "
       << to_binary(v.eallset, v.ebits, true) << '\n'; // mask of exponent value
  ...
  ostr << "significant field mask      : " << to_binary(v.hfmask, v.nbits, true)
       << '\n'; // mask of the signficicant field, i.e. hidden bit + fraction
                // bits
  ostr << "MSB fraction bit mask       : " << to_binary(v.fmsb, v.nbits, true)
       << '\n'; // mask of the most significant fraction bit
  ostr << "qNaN pattern                : "
       << to_binary(v.qnanmask, v.nbits, true) << '\n'; // mask of quiet NaN
  ostr << "sNaN pattern                : "
       << to_binary(v.snanmask, v.nbits, true)
...
template <typename Real, typename = typename std::enable_if<
                             std::is_floating_point<Real>::value, Real>::type>
...
template <typename Uint, typename Real> int _extractExponent(Real v) {
  ...
  // de-bias
  int e =
      static_cast<int>(raw) - static_cast<int>(ieee754_parameter<Real>::bias);
...
int scale(Real v) {
  ...
  } else if constexpr (sizeof(Real) ==
                       16) { // long double precision floating-point
...
template <typename Uint, typename Real> Uint _extractSignificant(Real v) {
  static_assert(sizeof(Real) == sizeof(Uint), "mismatched sizes");
  Uint raw{BitCast<Uint>(v)};
...
template <typename Real> void valueRepresentations(Real value) {
