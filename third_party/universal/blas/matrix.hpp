...
public:
  ...
  matrix(unsigned m, unsigned n, const Scalar *values)
      : _m{m}, _n{n}, data(values, values + m * n) {}
  ...
  */
  ...
  matrix &operator=(const Scalar &one) {
    ...
    for (unsigned i = 0; i < smallestDimension; ++i)
      data[i * _n + i] = one;
    return *this;
  }
  ...
  inline void resize(unsigned m, unsigned n) {
    _m = m;
    _n = n;
    data.resize(m * n);
  }
  const std::vector<Scalar> &get_data() const { return data; }
  // selectors
  inline unsigned rows() const { return _m; }
  inline unsigned cols() const { return _n; }
  inline std::pair<unsigned, unsigned> size() const {
...
template <typename Scalar> inline unsigned num_rows(const matrix<Scalar> &A) {
  return A.rows();
}
template <typename Scalar> inline unsigned num_cols(const matrix<Scalar> &A) {
  return A.cols();
}
template <typename Scalar>
inline std::pair<unsigned, unsigned> size(const matrix<Scalar> &A) {
  return A.size();
}
...
template <typename Scalar> Scalar maxelement(const matrix<Scalar> &A) {
  auto x = abs(A(0, 0));
...
template <typename Scalar> Scalar minelement(const matrix<Scalar> &A) {
  auto x = maxelement(A);
  for (size_t i = 0; i < num_rows(A); ++i) {
    for (size_t j = 0; j < num_cols(A); ++j) {
...
void disp(const matrix<Scalar> &A, const size_t COLWIDTH = 10) {
  for (size_t i = 0; i < num_rows(A); ++i) {
    for (size_t j = 0; j < num_cols(A); ++j) {
      // std::cout <<std::setw(COLWIDTH) << A(i,j) << std::setw(COLWIDTH) <<
      // "\t" << std::fixed;
